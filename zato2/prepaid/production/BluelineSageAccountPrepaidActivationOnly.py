...
class BluelineSageAccountPrepaidActivationOnly(Service):
    @staticmethod
    def get_name():
        return "blueline.sage.account.prepaid.activation.only"
    ...
    def handle(self):
        ...
        if self.request.input.transaction_date:
            transaction_date = dt.date(
                dt.strptime(self.request.input.transaction_date, "%Y-%m-%d")
            )
        else:
            transaction_date = dt.date(dt.now() - timedelta(days=1))
        try:
            ...
            if connection:
                ...
                report_param = self._build_report_param(
                ...
                self.response.payload.message = {
                    "transactions summary": totals,
                    "sage insertion": "SUCCESS",
                }
            else:
                self.response.payload.code = 502
                self.response.payload.status = "Bad Gateway"
                self.response.payload.message = "Unable to connect to Sage"
        except Exception as e:
            logging.error(
                "[BluelineSageAccountPrepaid] {}\n".format(
                    traceback.format_exc()
                )
            )
            self._send_error_report(
                self.request.input, str(traceback.format_exc()))
            self.response.payload.code = 500
            self.response.payload.status = "Internal Server Error"
            self.response.payload.message = e
        logging.info(
            (
                "[BluelineSageAccountPrepaid] Final result returned by the "
                "process :\n"
                "> code: {}\n"
                "> status: {}\n"
                "> message: {}"
