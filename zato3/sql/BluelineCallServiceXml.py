...
class BluelineCallServiceXml(Service):
    ...
    def handle(self):  # pylint: disable=missing-function-docstring
        ...
        #     self.response.payload = xml_out
            ...
            # Parse the response elements
            status = literal_eval(str(status))
            sql_property = literal_eval(str(sql_property))
            value_result = literal_eval(str(value_result))
            ...
            logging.info(
                f"[BluelineCallServiceXml][blueline.call.service.sql] SQL Property : {sql_property}\nstatus : {status}\nerrorNum : {errorNum}\nvalue_result : {value_result}"
            )
