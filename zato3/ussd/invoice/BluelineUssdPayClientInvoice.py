...
# Global Definitions
blueline_service_name = 'blueline.ussd.pay.client.invoice'

class BluelineUssdPayClientInvoice(Service):
    
    name = blueline_service_name

    class SimpleIO:
        input_required = (
            'operator', 'caller_num', 'fact_num', 'fact_cout', 'trans_ref'
        )
    ...
    def handle_POST(self):
        ...
        fact_num = self.request.input.fact_num
        fact_cout = self.request.input.fact_cout
        trans_ref = self.request.input.trans_ref
        
        ret_result = None
        log_message = 'pay invoice '
        ...
        logging.error('[BluelineUssdPayClientInvoice][Redis][check_token] : {}'.format(token_value))
        if not token_value:
            ret_code = 401
            ret_msg = 'Unauthorized'
            ...
            try:
                ...
                logging.error('[BluelineUssdPayClientInvoice][send data to bluebase] error : {}'.format(exception))
                ret_code = 500
                ret_msg = 'Internal Server Error'
            else:
                parsed_response = xmltodict.parse(bluebase_response)
                logging.error('_________________ parsed_response from bluebase : {}_____________'.format(parsed_response))
                if str(parsed_response['root']['header']['errorNum']) == '0':
                    ret_result = parsed_response['root']['data']
                    if 'status' in ret_result and str(ret_result['status']) == 'Vrai':
                        ret_result['status'] = 'paiement facture fait.'
                else:
        ...
        bluebase_payload = {
            "root": {
                ...
                "data": {
                    ...
                    "fact_num" : dicts["fact_num"],
                    "fact_cout" : dicts["fact_cout"],
                    "trans_ref" : dicts["trans_ref"]
                },
            }
        }
        try:
            with self.outgoing.soap.get("BluelineBluebaseS4DService").conn.client() as client:
                self.logger.info(
                    f"payload sent to s4d 'ussd_fact_paiement' : {bluebase_payload}"
                )
                data_xml = xmltodict.unparse(bluebase_payload)
                self.logger.info(
                    f"payload xml sent to s4d 'ussd_fact_paiement' : {data_xml}"
                )
            ...
            self.invoke(
                "blueline.send.email",
                {
                    ...
                    'title': "[BluelineUssdPayClientInvoice] USSD Paiement Facture | Erreur ",
                },
                as_bunch=True,
            )
            logging.info(
                "[BluelineUssdPayClientInvoice] Error report sent by email to "
                "dev@si.blueline.mg\n"
            )
            raise ValueError(e)